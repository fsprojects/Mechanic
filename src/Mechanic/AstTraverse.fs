// Copied from https://github.com/fsharp/FSharp.Compiler.Service/blob/13ecd8d4d080465bce4f49de72e4c13c6005e842/src/fsharp/vs/ServiceParseTreeWalk.fs and altered to walk through whole tree
//----------------------------------------------------
...
module AstTraversal =
    ...
    type AstVisitorBase<'T>() =
        ...
        abstract VisitLetOrUse : TraversePath * SynBinding list * range -> 'T option
        default this.VisitLetOrUse (_, _, _) = None
    ...
    let pick pos (outerRange:range) (_debugObj:obj) (diveResults:list<range*_>) =
        ...
        | [] -> None
        | _ ->
        diveResults |> List.map snd |> List.map (fun x -> x()) |> List.tryHead |> Option.bind id

    /// traverse an implementation file walking all the way down to SynExpr or TypeAbbrev at a particular location
    ///
    let Traverse(parseTree: ParsedInput, visitor:AstVisitorBase<'T>) =
        let pos = parseTree.Range.Start
        ...
        let rec traverseSynModuleDecl path (decl:SynModuleDecl) =
            ...
            let defaultTraverse m = 
                ...
                | SynModuleDecl.Let(_, synBindingList, range) ->
                    match visitor.VisitLetOrUse(path, synBindingList, range) with
                    | Some x -> Some x
        ...
        and traverseSynBinding path b =
            ...
            visitor.VisitBinding(path,defaultTraverse,b)
